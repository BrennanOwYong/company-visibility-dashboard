...
`--links` is the ticket's explicit reference set — it becomes the `## References` block the builder and validator read, so it must name every relevant file and only the relevant ones. Do NOT rely on name-matching (files are deliberately named differently: the `pairing` ticket's files are `spec/pairing.md`, `contracts/acceptance/pairing.md`, `technical/feasibility/pairing.md`, `technical/research/tailscale-pairing.md`, and the `iface/pairing-handshake.md` + `iface/wire-transport.md` seams — a name search would miss `tailscale-pairing`). You already know the grouping because you authored the spec: its `research:` line names the research docs, its `reconciliation:` line names the architecture elements/ADRs, and its Logic/Needs name the seams. Enumerate, per ticket:
- the feature's `knowledge/prd/product.md` section, `knowledge/contracts/acceptance/<feature>.md`, `knowledge/spec/<feature>.md`, and `knowledge/technical/feasibility/<feature>.md`
- every `knowledge/technical/research/<tech>.md` the spec's `research:` line cites
- every `knowledge/contracts/iface/<seam>.md` this ticket produces or consumes
- `knowledge/architecture.md` and any `[[module]]` backlinks
kanban-create warns if any listed path does not exist — treat that warning as a defect in your enumeration, not noise.
